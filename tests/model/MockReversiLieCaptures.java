...
/**
 * Mock that lies about the amount of captures.
 */
public class MockReversiLieCaptures implements IReversiModel {
  StringBuilder log;
  IReversiModel delegate;

  public MockReversiLieCaptures(IReversiModel delegate, StringBuilder log) {
  ...
  public int countCaptures(IPosition pos, Cell hex) {
    int count = 0;
    if (pos.getQ() == 2 && pos.getS() == -1 && pos.getR() == -1) {
      count = 5;
    }
    log.append(count + "\n");
    return count;
